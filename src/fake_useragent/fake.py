...
class FakeUserAgent:
    def __init__(
        ...
        if safe_attrs:
            ...
            assert all(str_types_safe_attrs), "safe_attrs must be list\\tuple\\set of strings or unicode"
    ...
    def _filter_useragents(self, request=None):
        ...
        if request:
            filtered_useragents = list(filter(lambda x: x["browser"] == request, filtered_useragents))
    ...
    def getBrowser(self, request):
        try:
            ...
            if self.fallback is None:
                raise FakeUserAgentError(f"Error occurred during getting browser: {request}")  # noqa
            else:
                logger.warning(
    ...
    def __getattr__(self, attr):
        ...
        try:
            ...
            if self.fallback is None:
                raise FakeUserAgentError(f"Error occurred during getting browser: {attr}")  # noqa
            else:
                logger.warning(
